package org.dialog.custom.hostobjects.northbound;
...
import org.dialog.custom.hostobjects.*;
...
import org.dialog.custom.hostobjects.util.*;
...
import java.util.*;
import org.dialog.custom.dao.TxCardDAO;
...
//		                                                              .getAPIManagerConfiguration();
    ...
    public static Map<Application, Set<BillingSubscription>> getBillingSubscriptionsForUser(String username, String year, String month)
    ...
    private static boolean isSubscriptionValidForMonth(SubscribedAPI subAPI, String year, String month) throws
            APIManagementException, APIMgtUsageQueryServiceClientException {
        java.util.Date createdTime = BillingDataAccessObject.getSubscriptionCreatedTime(subAPI
                .getApplication().getId(), subAPI.getApiId());
    ...
    public static SubscribedAPI getBillingSubscriptionsForUserApp(String username, String application, String txnapi)
    ...
    public static void applyChargingPlan(Map<Application, Set<BillingSubscription>> billingSubs,
            Map<RateKey, ChargeRate> rateCard, String year, String month) throws APIMgtUsageQueryServiceClientException, APIManagementException {
        ...
        for (Map.Entry<Application, Set<BillingSubscription>> entry : entries) {
            ...
            for (BillingSubscription subscription : subscriptions) {
                ...
                subscription.setApiIdInt(BillingDataAccessObject.getApiId(subscription.getApiId()));
    ...
    private static void billComponent(ChargeRate rate, Map<RateKey, ChargeRate> rateCard, BillingSubscription.OperatorSubscription operatorSubscription, String subsYear,
            String subsMonth, Application application, String ApiName, String apiVersion, Map.Entry<CategoryCharge, BilledCharge> categoryEntry, int appId, int apiId, String subsId) throws APIManagementException,
        ...
        switch (rate.getType()) {
            ...
            case QUOTA:
                ...
                if (rateAttributes == null || !rateAttributes.containsKey(HostObjectConstants.RATE_ATTRIBUTE_MAX_COUNT.toString())
                        || !rateAttributes.containsKey(HostObjectConstants.RATE_ATTRIBUTE_EXCESS_RATE.toString()) || !rateAttributes.containsKey(HostObjectConstants.RATE_ATTRIBUTE_DEFAULT_RATE.toString())) {
                    throw new APIManagementException("Attributes required for QUOTA charging are not specified in rate-card.xml");
                }
                int maxCount = Integer.parseInt(rateAttributes.get(HostObjectConstants.RATE_ATTRIBUTE_MAX_COUNT.toString()));
                BigDecimal excessRate = new BigDecimal(rateAttributes.get(HostObjectConstants.RATE_ATTRIBUTE_EXCESS_RATE.toString()));
                BigDecimal defaultRate = new BigDecimal(rateAttributes.get(HostObjectConstants.RATE_ATTRIBUTE_DEFAULT_RATE.toString()));
            ...
            case PERCENTAGE:
                ...
                if (prodKey != null) {
                    ...
                    Set<PaymentRequestDTO> paymentRequestSet = BillingDataAccessObject.getNbPaymentAmounts(Short.parseShort(subsYear), Short.parseShort(subsMonth),
                            consumerKey, api_version, operatorSubscription.getOperationId(), categoryEntry.getKey().getCategory(), categoryEntry.getKey().getSubcategory());
                    applyChargesForPaymentApi(operatorSubscription, paymentRequestSet, categoryEntry, appId, apiId, subsId);
                }
                break;
            ...
            case PER_REQUEST:
                applyChargesWithTax(subsYear, subsMonth, application, ApiName, apiVersion, operatorSubscription, categoryEntry, rate);
                break;
    ...
    private static Object getRateSubcategory(ChargeRate rate, String billCategory, String billSubCategory) throws APIManagementException {
        ...
        if ((billCategory != null) && (!billCategory.isEmpty())) {
            if (!rate.getCategories().containsKey(billCategory)) {
                throw new APIManagementException("Attributes required for charging are not specified in rate-card.xml");
            }
    ...
    private static void calculateChargeForSubscription(BillingSubscription subscription, Map<RateKey, ChargeRate> rateCard) throws APIManagementException,
            APIMgtUsageQueryServiceClientException {

        for (BillingSubscription.OperatorSubscription operatorSubscription : subscription.getOperatorSubscriptionList()) {
            ...
            for (Map.Entry<CategoryCharge, BilledCharge> entry : map.entrySet()) {
                //CategoryCharge key = entry.getKey();
                //BilledCharge billedCharge = entry.getValue();
                billComponent(rate, rateCard, operatorSubscription, subscription.getYear(), subscription.getMonth(), subscription.getApplication(), subscription.getApiId().getApiName(),
                        subscription.getApiId().getVersion(), entry, subscription.getApplication().getId(), subscription.getApiIdInt(), subscription.getSubscriber().getName());
    ...
    private static void applyChargesForPaymentApi(BillingSubscription.OperatorSubscription opSubscription,
            Set<PaymentRequestDTO> paymentRequestSet, Map.Entry<CategoryCharge, BilledCharge> categoryEntry, int appId, int apiId, String subId) throws
            APIManagementException, APIMgtUsageQueryServiceClientException {
        ...
        List<Tax> taxList = TaxDataAccessObject.getTaxesForTaxList(rate.getTaxList());
        ...
        for (PaymentRequestDTO paymentRequest : paymentRequestSet) {
            ...
            if (CategoryBased) {
                ...
                if (commisionMap.containsKey(merchant)) {
                    ...
                    throw new APIManagementException("Payment Categoreis required for MERCHANT based charging are not specified in rate-card.xml");
                }
            }
    ...
    private static void applyPaymentChargesByCategory(BillingSubscription.OperatorSubscription opSubscription, CategoryCharge categoryCharge,
            Set<PaymentRequestDTO> paymentRequestSet) throws
            APIManagementException, APIMgtUsageQueryServiceClientException {
        ...
        for (PaymentRequestDTO paymentRequest : paymentRequestSet) {
            ...
            if (rateCategories == null) {
                throw new APIManagementException("Payment Categoreis required for QUOTA charging are not specified in rate-card.xml");
            }
            BigDecimal catpercent = rate.getValue().divide(new BigDecimal(100));
            ...
            /*
             ...
             price = paymentRequest.getAmount().multiply(catpercent);
             totalPrice = totalPrice.add(price);
             */
            //<<TODO if commision is cate/subcate based calculate category bases commisions else merchant based commision
        ...
        //BigDecimal price = totalCharge.multiply(percentage);
        //opSubscription.setPrice(totalPrice);
        //opSubscription.setCredit(totalCharge.subtract(totalPrice));

        //opSubscription.setTaxValue(totalTax);
    }

    private static void applyChargesWithTax(String apiYear, String apiMonth, Application application, String apiName, String apiVersion,
            BillingSubscription.OperatorSubscription operatorSub, Map.Entry<CategoryCharge, BilledCharge> CatEntry, ChargeRate rate) throws
            APIManagementException, APIMgtUsageQueryServiceClientException {
        ...
        if (prodKey != null) {
            String api_version = apiName + ":v" + apiVersion;
            requestTimes = TaxDataAccessObject.getNbAPIRequestTimesForSubscription(Short.parseShort(year),
                    Short.parseShort(month), apiName,
                    api_version, prodKey.getConsumerKey(), operatorSub.getOperationId(), CatEntry.getKey().getCategory(), CatEntry.getKey().getSubcategory());
        }
    ...
    private static void applyTaxForBlockCharging(Map.Entry<CategoryCharge, BilledCharge> CatEntry, ChargeRate rate, String year,
            String month) throws APIManagementException,
            APIMgtUsageQueryServiceClientException {
    ...
    public static Map<Application, Set<BillingSubscription>> chargeSubscriberForMonth(String subscriber,
            ...
            APIMgtUsageQueryServiceClientException,
            SQLException {
        Map<Application, Set<BillingSubscription>> billingSubs = getBillingSubscriptionsForUser(subscriber, year, month);
        //populateAPICounts(billingSubs, year, month, subscriber); // Moved inside charge plan application.
...
//                BillingDataAccessObject.printSouthboundTraffic();
    ...
    private static void populateOperatorDetailsOfSubscription(BillingSubscription subscription, Map<RateKey, ChargeRate> rateCard) throws APIManagementException, APIMgtUsageQueryServiceClientException {
        ...
        List<OperatorDetailsEntity> operatorMap = BillingDataAccessObject.getDetailsOfSubscription(appId, apiId);
        for (OperatorDetailsEntity operatorDetail : operatorMap) {
            ...
            //String operationName = BillingDataAccessObject.getOperationNameById(operationId);
            log.debug("appId:" + appId + ",apiId:" + apiId + ",operator:" + operator + ",rateName:" + rateName + ",operation:" + operationId);

            BillingSubscription.OperatorSubscription opSub = new BillingSubscription.OperatorSubscription(operator);
            ChargeRate rate = rateCard.get(new RateKey(operator, subscription.getApiId().getApiName(), rateName, String.valueOf(operationId)));
            if (rate == null) {
    ...
    /*   private static void populateAPICountsOfSubscription(BillingSubscription subscription)
     throws APIManagementException, APIMgtUsageQueryServiceClientException {
     ...
     APIKey prodKey = getAppKey(application, APIConstants.API_KEY_TYPE_PRODUCTION);
     Map<CategoryCharge, Integer> usageCounts = new HashMap<CategoryCharge, Integer>();
     if (prodKey != null) {
     usageCounts = BillingDataAccessObject.getAPICountsForSubscription(prodKey.getConsumerKey(),
     ...
     for (BillingSubscription.OperatorSubscription operatorSubscription : subscription.getOperatorSubscriptionList()) {
     String operator = operatorSubscription.getOperator();
     if (usageCounts.containsKey(operator)) {
     operatorSubscription.setCount(usageCounts.get(operator));
     }
     }
     } */
    private static void populateCategoryViseAPICounts(BillingSubscription subscription)
            throws APIManagementException, APIMgtUsageQueryServiceClientException {
        ...
        for (BillingSubscription.OperatorSubscription operatorSubscription : subscription.getOperatorSubscriptionList()) {
            ...
            if (prodKey != null) {
                categoryCharges = BillingDataAccessObject.getNorthboundAPICountsForSubscription(prodKey.getConsumerKey(),
                        Short.parseShort(subscription.getYear()), Short.parseShort(subscription.getMonth()),
                        subscription.getApiId().getApiName(), subscription.getApiId().getVersion(), operatorSubscription.getOperationId());
            }
    ...
    public static Map<String, List<APIResponseDTO>> getAllResponseTimes(String opName, String username, String application, String appId,
        ...
        if (username.equals(ALL_SUBSCRIBERS_KEYWORD)) {
            ...
            for (API api : allAPIs) {
                ...
                List<APIResponseDTO> responseDTOs = BillingDataAccessObject.getAllResponseTimesForAPI(opName, appId, nameVersion, fromDate, toDate);
    ...
    public static FileWriter getCustomApiTrafficReportLocation(String fromDate, String toDate, String subscriberName, String operator, String api) throws IOException {

        String carbonHome = System.getProperty("carbon.home");
        String fileName = fromDate + "-" + toDate + "-" + subscriberName + "-" + operator + "-" + api + "-" + ".csv";
        File reports = new File(carbonHome + "/reports/");
    ...
    private static File getCustomCSVFile(String fromDate, String toDate, String subscriberName, String operator, String api) throws IOException {
    ...
    public static NativeArray generateCostperApisummary(boolean isPersistReport,
            String subscriberName, String period, String opcode, String application)
            throws IOException,
            SQLException,
            APIMgtUsageQueryServiceClientException,
    ...
    public static NativeArray generateCustomrCareDataReport(boolean isPersistReport,
            String fromDate, String toDate, String msisdn, String subscriberName, String operator, String app, String api, String stLimit, String endLimit, String timeOffset) throws IOException,
            SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {

        NativeArray nativeArray = new NativeArray(0);
        List<String[]> user_data_all = NbHostObjectUtils.getFilteredCustomerCareReport(fromDate, toDate, msisdn, subscriberName, operator, app, api, stLimit, endLimit, timeOffset);
    ...
    public static String generateCustomrCareDataRecordCount(boolean isPersistReport,
            String fromDate, String toDate, String msisdn, String subscriberName, String operator, String app, String api) throws IOException,
            SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {

        String user_data_count = BillingDataAccessObject.getCustomerCareReportDataCount(fromDate, toDate, msisdn, subscriberName, operator, app, api);

        return user_data_count;
    }

    // Hira added - START
    public static NativeArray generateCustomTrafficReport(boolean isPersistReport,
            String fromDate, String toDate, String subscriberName,
            String operator, String api, String timeOffset, String resType) throws IOException,
        ...
        if (currentDao != null && isPersistReport) {
            ...
            ResultSet rs = BillingDataAccessObject.getTxLogDataNB(fromDate, toDate, subscriberName, operator, operationType, resType);
            ...
            while (rs.next()) {
                for (int d = 0; d < dataList.size(); d++) {
                    ...
                    if (tempDL.getManipulationType().equals("FIELD")) {
                        ...
                        //fileWriter.write(String.format("%s", rs.getString(tempDL.getManipulationField())));
                        try {
                        ...
                        String convertedTime = BillingDataAccessObject.convertToLocalTime(timeOffset,timeInDb);
                        ...
                        //fileWriter.write(String.format("%s", tempDL.getManipulationRegEx()));
    ...
    public static NativeArray generateReportofSubscriber(boolean isPersistReport,
        ...
        if (isPersistReport) {
            ...
            fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s,%s, %s, %s, %s, %s, %s %n", "Subscriber",
                    "Application", "API", "Version", "Operation", "Plan", "Currency", "Merchant", "Category", "SubCategory", "Tier", "Count",
                    "Operator Charges", "Usage Charge", "Tax", "Credit", "Grand Total"));
        }

        NativeArray subscribers = new NativeArray(0);

        if (subscriberName.equals(ALL_SUBSCRIBERS_KEYWORD)) {
            List<String> subscriberNames = NbHostObjectUtils.getAllSubscribers();

            for (String sub : subscriberNames) {
                Map<Application, Set<BillingSubscription>> billingSubs = NbHostObjectUtils.chargeSubscriberForMonth(sub,
                        year,
                        month,
                        rateCard);
                ...
                if (isPersistReport && fileWriter != null) {
                    fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s %n", sub, "", "",
                            "", "", "", "", "", "", "", "", "", "", "", "", ""));
                }
                ...
                for (Map.Entry<Application, Set<BillingSubscription>> billingEntries : billingSubs.entrySet()) {
                    ...
                    for (BillingSubscription billingSubscription : billingEntries.getValue()) {
                        ...
                        if (a == 0 && isPersistReport && fileWriter != null) {
                            fileWriter.write(String.format("%s, %s, %s,%s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s %n", "",
                                    applicationName, "", "", "", "", "", "", "", "", "", "", "", "", "", ""));
                        }
                        ...
                        for (BillingSubscription.OperatorSubscription operatorSub : billingSubscription
                            ...
                            subscriptionOperator.put("operationid", subscriptionOperator, BillingDataAccessObject.getOperationNameById(operatorSub.getOperationId()));
                            subscriptionOperator.put("ratename", subscriptionOperator, operatorSub.getRate().getName());
                            ...
                            subscriptionOperator.put("currency", subscriptionOperator, operatorSub.getRate().getCurrency());

                            subscriptionOperators.put(subscriptionOperators.size(), subscriptionOperators, subscriptionOperator);

                            if (isPersistReport && fileWriter != null) {
                                String currencyStr = operatorSub.getRate().getCurrency() + " ";
                                fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s %n", "", "",
                                        "", "", BillingDataAccessObject.getOperationNameById(operatorSub.getOperationId()), operatorSub.getRate().getName(), currencyStr,
                                ...
                                if (mtmap.size() > 0) {
                                    for (Map.Entry<String, BilledCharge> entry : mtmap.entrySet()) {
                                        fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s %n", "", "",
                                                "", "", "", "", currencyStr,
                                                nvlDefault(entry.getKey()), "", "", "", entry.getValue().getCount(), entry.getValue().getOpcom(), entry.getValue().getPrice(),
                                                entry.getValue().getTax(), entry.getValue().getAdscom()));
                                    }
                                } else {
                                    ...
                                    for (Map.Entry<CategoryCharge, BilledCharge> entry : ctmap.entrySet()) {
                                        fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s %n", "", "",
                                                "", "", "", "", "",
                                                "", nvlDefault(entry.getKey().getCategory()), nvlDefault(entry.getKey().getSubcategory()), "", entry.getValue().getCount(), entry.getValue().getOpcom(), entry.getValue().getPrice(),
                                                entry.getValue().getTax(), entry.getValue().getAdscom()));
                                        ...
                                        for (Map.Entry<String, BilledCharge> tientry : timap.entrySet()) {
                                            fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s %n", "", "",
                                                    "", "", "", "", "",
                                                    "", "", "", tientry.getKey(), tientry.getValue().getCount(), tientry.getValue().getOpcom(), tientry.getValue().getPrice(),
                                                    tientry.getValue().getTax(), tientry.getValue().getAdscom()));
                                        }
                ...
                if (isPersistReport && fileWriter != null) {
                    fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s,%s, %s, %s, %s, %s %n",
                            "", "", "", "", "", "", "", "", "", "", "", "Total Amount", acOpAmount, totalAmount, ataxAmout, acreditAmount, grandTotal));
                }
            ...
            Map<Application, Set<BillingSubscription>> billingSubs = NbHostObjectUtils.chargeSubscriberForMonth(subscriberName,
                    year,
                    month,
                    rateCard);
            if (isPersistReport) {
                fileWriter.write(String.format("%s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s %n", subscriberName, "", "",
                        "", "", "", "", "", "", "", "", "", "", "", "", ""));
            }
    ...
    public static Map<String, Integer> getOperatorbreakdown(String applicationid, String year, String month,
            String subscriber, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {

        String consumerkey = "%";
        if (applicationid != null) {
            consumerkey = BillingDataAccessObject.getApplicationconsumer(Integer.parseInt(applicationid), APIConstants.API_KEY_TYPE_PRODUCTION);
        }

        Map<String, Integer> usageCounts = BillingDataAccessObject.getAPICountsForApplicationOpco(consumerkey, year,
                month, subscriber, api);

        return usageCounts;

    }

    public static String getApplicationNameById(String applicationid) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {

        String appName = BillingDataAccessObject.getApplicationName(Integer.parseInt(applicationid), APIConstants.API_KEY_TYPE_PRODUCTION);

        return appName;

    }

    public static List<String[]> getTotalAPITrafficForPieChart(String fromDate, String toDate, String subscriber, String operator, int applicationId) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request = BillingDataAccessObject.getTotalAPITrafficForPieChart(fromDate, toDate, subscriber, operator, applicationId);
        return api_request;
    }

    public static List<String[]> getTotalAPITrafficForHistogram(String fromDate, String toDate, String subscriber, String operator, int applicationId, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request = BillingDataAccessObject.getTotalAPITrafficForHistogram(fromDate, toDate, subscriber, operator, applicationId, api);
        return api_request;
    }

    public static List<String[]> getOperatorWiseAPITrafficForPieChart(String fromDate, String toDate, String subscriber, String api, int applicationId) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request = BillingDataAccessObject.getOperatorWiseAPITrafficForPieChart(fromDate, toDate, subscriber, api, applicationId);
        return api_request;
    }

    public static List<String[]> getApprovalHistory(String fromDate, String toDate, String subscriber, String api, int applicationId, String operator) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request = BillingDataAccessObject.getApprovalHistory(fromDate, toDate, subscriber, api, applicationId, operator);
        return api_request;
    }

    public static List<Approval> getApprovalHistoryApp(int applicationId, String operator) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<Approval> api_request = BillingDataAccessObject.getApprovalHistoryApp(applicationId, operator);
        return api_request;
    }

    public static List<String[]> getAllAPIs(String fromDate, String toDate, String subscriber, String operator, int applicationId, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> apis = BillingDataAccessObject.getAllAPIs(fromDate, toDate, subscriber, operator, applicationId, api);
        return apis;
    }

    public static List<String[]> getAllErrorResponseCodes(String fromDate, String toDate, String subscriber, String operator, int applicationId, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> resCodes = BillingDataAccessObject.getAllErrorResponseCodes(fromDate, toDate, subscriber, operator, applicationId, api);
        return resCodes;
    }

    public static List<String> getSubscribersByOperator(String operatorName) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<Integer> applicationIds = AxiataDataAccessObject.getApplicationsByOperator(operatorName);
    ...
    /*public static List<String[]> getApplicationsByOperator(String operatorName) throws SQLException,
     APIMgtUsageQueryServiceClientException,
     APIManagementException {
     List<Integer> applicationIds = AxiataDataAccessObject.getApplicationsByOperator(operatorName);
     ...
     applications.add(tempApplicationDetails);
     }
     return applications;
     }*/
    public static List<String[]> getApplicationsBySubscriber(String subscriberName) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> applicationsList = new ArrayList<String[]>();
        ApiMgtDAO apiMgtDAO = new ApiMgtDAO();
        Application[] applications = apiMgtDAO.getApplications(new Subscriber(subscriberName));
    ...
    public static List<String> getOperatorsBySubscriber(String subscriberName) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        ...
        if (applications != null && applications.length > 0) {
            for (Application application : applications) {
                ...
                tempOperatorNames = AxiataDataAccessObject.getOperatorNamesByApplication(tempApplicationId);
    ...
    public static List<String> getAPIsBySubscriber(String subscriberName) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException,
            IdentityException {
    ...
    /*public static List<String[]> getAPIWiseTrafficForHistogram(String fromDate, String toDate, String subscriber, String operator, String api) throws SQLException,
     APIMgtUsageQueryServiceClientException,
     APIManagementException {
     List<String[]> api_request = BillingDataAccessObject.getAPIWiseTrafficForHistogram(fromDate, toDate, subscriber, operator, api);
     return api_request;
     } */
    public static List<String[]> getAPIWiseTrafficForReport(String fromDate, String toDate, String subscriber, String operator, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request_data = BillingDataAccessObject.getAPIWiseTrafficForReport(fromDate, toDate, subscriber, operator, api);
        return api_request_data;
    }

    public static List<String[]> getFilteredCustomerCareReport(String fromDate, String toDate, String msisdn, String subscriber, String operator, String app, String api, String stLimit, String endLimit, String timeOffset) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request_data = BillingDataAccessObject.getCustomerCareReportData(fromDate, toDate, msisdn, subscriber, operator, app, api, stLimit, endLimit, timeOffset);
        return api_request_data;
    }

    public static List<String[]> getAPIWiseTrafficForReportCharging(String fromDate, String toDate, String subscriber, String operator, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> charging_request_data = BillingDataAccessObject.getAPIWiseTrafficForReportCharging(fromDate, toDate, subscriber, operator, api);
...
//    }
    ...
    public static String getCustomReport(String fromDate, String toDate, String subscriberName, String operator, String api) {
        String fileContent = "";
        BufferedReader bufferedReader = null;
        try {
    ...
    public static List<String[]> getErrorResponseCodesForPieChart(String fromDate, String toDate, String subscriber, String operator, int applicationId, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_request = BillingDataAccessObject.getErrorResponseCodesForPieChart(fromDate, toDate, subscriber, operator, applicationId, api);
        return api_request;
    }

    public static List<String[]> getErrorResponseCodesForHistogram(String fromDate, String toDate, String subscriber, String operator, int applicationId, String api) throws SQLException,
            APIMgtUsageQueryServiceClientException,
            APIManagementException {
        List<String[]> api_response_codes = BillingDataAccessObject.getErrorResponseCodesForHistogram(fromDate, toDate, subscriber, operator, applicationId, api);
     ...
     * @throws APIManagementException if an error occurs when loading rate card
     * from the registry
     */
    public static Map<RateKey, ChargeRate> getRateCard() throws APIManagementException {
        ...
        try {
            ...
            if (registry.resourceExists(HostObjectConstants.NB_RATE_CARD_LOCATION)) {
                Resource resource = registry.get(HostObjectConstants.NB_RATE_CARD_LOCATION);
                ...
                Iterator apis = opsElement.getChildrenWithName(HostObjectConstants.API_ELEMENT);
                while (apis.hasNext()) {
                    ...
                    while (operations.hasNext()) {
                        ...
                        while (rates.hasNext()) {
                            ...
                            OMElement rootAttributes = rate.getFirstChildWithName(HostObjectConstants.RATE_ATTRIBUTES_ELEMENT);
                            if (rootAttributes != null) {
                                Map<String, String> attributesMap = new HashMap<String, String>();
                                for (Iterator childElements = rootAttributes.getChildElements(); childElements.hasNext();) {
                            ...
                            if (usageTiers != null) {
                                ...
                                while (tiers.hasNext()) {
                                    ...
                                    String tierName = tier.getAttributeValue(HostObjectConstants.TIER_NAME_ELEMENT);
                            ...
                            //roshan 2015/05/24 moved inside the subslist  
                            ...
                            while (categoryIter.hasNext()) {
                                ...
                                if (category != null) {
                                    ...
                                    while (subCategories.hasNext()) {
                                        ...
                                        //commented by roshan 2016/05/24
                                        ...
                                        OMElement subAttributes = subCategory.getFirstChildWithName(HostObjectConstants.RATE_ATTRIBUTES_ELEMENT);
                                        ...
                                        //2015/05/24 commented and moved up
                                        //subCategoryEntityMap.put(subCategoryname, subCategoriesMap);
                                        //categoryEntityMap.put(categoryName, subCategoryEntityMap);
     ...
     * from the registry
     */
    public static List<ChargeRate> getRatesForOperatorApi(String operatorName, String apiName) throws APIManagementException {
        ...
        try {
            ...
            if (registry.resourceExists(HostObjectConstants.NB_RATE_CARD_LOCATION)) {
                ...
                String xpath = "//" + HostObjectConstants.OPERATORS_ELEMENT + "/" + HostObjectConstants.OPERATOR_ELEMENT
                        .getLocalPart() + "[@" + HostObjectConstants.NAME_ATTRIBUTE.getLocalPart() + "='"
                        + operatorName + "']/" + HostObjectConstants.API_ELEMENT
                        .getLocalPart() + "[@" + HostObjectConstants.NAME_ATTRIBUTE.getLocalPart() + "='" + apiName + "']/" + HostObjectConstants.RATE_ELEMENT.getLocalPart();
                ...
                for (Object rateNode : rateNodes) {
                    ...
                    if (categories != null) {
                        ...
                        for (Iterator childElements = categories.getChildElements(); childElements.hasNext();) {
                            OMElement subCatElem = (OMElement) childElements.next();
                            String subCategoryName = subCatElem.getAttributeValue(HostObjectConstants.NAME_ATTRIBUTE);
                            BigDecimal subCategoryRate = BigDecimal.valueOf(Integer.valueOf(subCatElem.getAttributeValue(HostObjectConstants.TIER_RATE_ELEMENT)));
                            categoriesMap.put(subCategoryName, subCategoryRate);
                        }
                        categoryEntity.setSubCategoryMap(categoriesMap);
                        //chargeRate.setCategories(categoryEntity);
                    }

                    OMElement taxesElement = rate.getFirstChildWithName(HostObjectConstants.RATE_TAXES_ELEMENT);
                    if (taxesElement != null) {
                        List<String> taxList = new ArrayList<String>();
                        for (Iterator taxes = taxesElement.getChildrenWithName(HostObjectConstants.RATE_TAX_ELEMENT); taxes.hasNext();) {
    ...
    public static List<APIResponseDTO> getTotalAPIResponseTimeForLineChart(String fromDate, String toDate, String subscriber, String operator,
            String timeRange) throws SQLException, APIMgtUsageQueryServiceClientException, APIManagementException {

        List<APIResponseDTO> apiResponse = BillingDataAccessObject.getAllResponseTimesForAllAPIs(operator, subscriber, fromDate,
                toDate, timeRange);
        return apiResponse;
    }
    ...
    public static Map<String, List<APIResponseDTO>> getAllResponseTimesByDate(String opName, String username,
        ...
        for (API api : allAPIs) {
            ...
            List<APIResponseDTO> responseDTOs = BillingDataAccessObject.getAllResponseTimesForAPIbyDate(opName, username, fromDate, toDate, apiName);
    ...
    public static Map<String, String[]> getTimeConsumptionForAllAPIs(String opName, String username,
        ...
        for (API api : allAPIs) {
            String apiName = api.getId().getApiName();
            String[] responseData = BillingDataAccessObject.getTimeConsumptionByAPI(opName, username, fromDate, toDate, apiName);
    ...
    private static void calculateTiersCharges(List<UsageTiers> selectedUsageTier, Map<RateKey, ChargeRate> rateCard, int totalRequestCount, int tierCount, BillingSubscription.OperatorSubscription operatorSubscription, String subsYear,
            String subsMonth, Application application, String ApiName, String apiVersion, Map.Entry<CategoryCharge, BilledCharge> categoryEntry, int appId, int apiId, String subsId) throws APIManagementException,
            APIMgtUsageQueryServiceClientException {
        ...
        for (UsageTiers usageTiers : selectedUsageTier) {
            ...
            ChargeRate subRate = rateCard.get(new RateKey(operatorSubscription.getOperator(), ApiName, rateId, String.valueOf(operationId)));
            if (subRate == null) {
                throw new APIManagementException("Attributes required for Charging are not specified in rate-card.xml");
            }
            categoryEntry.setValue(tempBilledCharge);
            billComponent(subRate, rateCard, operatorSubscription, subsYear, subsMonth, application, ApiName, apiVersion, categoryEntry, appId, apiId, subsId);
            ...
            //CategoryCharge.addCount(categoryEntry.getValue().getCount());

            //accumilate multitier chargers
            if (CategoryCharge.getTierCharges().containsKey(logTier)) {

                CategoryCharge.getTierCharges().get(logTier).addPrice(categoryEntry.getValue().getPrice());
                CategoryCharge.getTierCharges().get(logTier).addTax(categoryEntry.getValue().getTax());
                CategoryCharge.getTierCharges().get(logTier).addCount(categoryEntry.getValue().getCount());
            } else {
