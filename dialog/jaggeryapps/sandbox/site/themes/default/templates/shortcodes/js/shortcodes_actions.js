$(document).ready(function(){
	...
	$("#add_new_row_button").click(function(){
        ...
        col2 = $('<td class="editable_td"><input type="text" class="" placeholder="Ex: Tesing number" /></td>');
        ...
        row.append(col1, col2, col3).prependTo("#numbers_data_table");
    ...
    $('#data_class_body').on( 'click', 'a.edit_tbl_icon', function () {
        ...
        $(closestTR.find('td').children()[0]).attr("class", "required number shortcode_exist");
        $(closestTR.find('td').children()[1]).attr("class", "");
  ...
  //Save new record
    $('#data_class_body').on( 'click', 'a.save_data_icon', function () {
        ...
        var shortcode = $(closestTr.find('td').children()[0]).val();
        var description = $(closestTr.find('td').children()[1]).val();
        
        jagg.post(request_url, {'action':"SaveShortCode", 'userid':userid, 'shortcode':shortcode , 'description':description}, function (result) {
            ...
            if(!result.error){
                ...
                if(responseOk == "true"){
                    ...
                    closestTr.find('td:nth-child(1)').text(shortcode);
                    closestTr.find('td:nth-child(2)').text(description);
                    
                    $('#tableSuccess').show('fast');
                    $('#tableSuccessSpan').html('<strong>Success!</strong><br />' + "Sender Address data saved successfully");
                    $('#tableSuccess').delay(4000).hide('fast');
        
                    //Setting action buttons
                    closestTr.find('td:nth-child(3)').children().html("");
                    ...
                    closestTr.find('td:nth-child(3)').prepend(keyTag, data1, data2, data3, data4);
                } else {
            		$('#tableError').show('fast');
                    $('#tableErrorSpan').html('<strong>Error!</strong><br />Server Error occurs while adding <i>'+shortcode+'</i>! Please re-try.');
                    $('#tableError').delay(4000).hide('fast');
                }
            } else {
            	$('#tableError').show('fast');
                $('#tableErrorSpan').html('<strong>Error!</strong><br /> Error in saving Sender Address '+shortcode+': '+ result.message);
                $('#tableError').delay(4000).hide('fast');
            }
        }, "json");
        }
  ...
  //Delete data and remove row
    $('#data_class_body').on( 'click', 'a.delete_tbl_icon', function () {
    	...
    	jagg.post(request_url, {'action':"DeleteShortCode", 'id':key}, function (result) {
    		if(!result.error){

                $('#tableSuccess').show('fast');
                $('#tableSuccessSpan').html('<strong>Success!</strong><br />' + "Sender Address data updated successfully");
                $('#tableSuccess').delay(4000).hide('fast');
                
    			closestTr.remove();
    		} else {
            	$('#tableError').show('fast');
                $('#tableErrorSpan').html('<strong>Error!</strong><br /> Error in deleting Sender Address: '+ result.message);
                $('#tableError').delay(4000).hide('fast');
    		}
    	}, "json");
    	setTablePagination(0);
  ...
  //Save modified row data
    $('#data_class_body').on( 'click', 'a.save_edit_icon', function () {
        ...
        var code = $(closestTR.find('td').children()[0]).val();
        var description = $(closestTR.find('td').children()[1]).val();
        
        var request_url = "/site/blocks/shortcodes/ajax/shortcodefunctions.jag";
        var responseOk = false;
        jagg.post(request_url, {'action':"EditShortCode", 'id':key, 'code':code , 'description':description}, function (result) {
            //alert(result.data);
            if(!result.error){
                ...
                closestTR.find('td:nth-child(1)').text(code);
                closestTR.find('td:nth-child(2)').text(description);
                
                $('#tableSuccess').show('fast');
                $('#tableSuccessSpan').html('<strong>Success!</strong><br />' + "Sender Address updated successfully");
                $('#tableSuccess').delay(4000).hide('fast');
                ...
                closestTR.find('td:last-child .reset_edit_icon').hide();
            		...
            		$('#tableError').show('fast');
                    $('#tableErrorSpan').html('<strong>Error!</strong><br />Server Error occurs while updating <i>'+code+'</i>! Please re-try.');
                    $('#tableError').delay(4000).hide('fast');
            	}
            } else {
            	$('#tableError').show('fast');
                $('#tableErrorSpan').html('<strong>Error!</strong><br /> Error in updating Short Code '+code+': '+ result.message);
                $('#tableError').delay(4000).hide('fast');
            }
        }, "json");
        }
  ...
  //Reset clicked row data
    $('#data_class_body').on( 'click', 'a.reset_edit_icon', function () {
        ...
        jagg.post(request_url, {'action':"SearchCode", 'id':key}, function (result) {
    		if(!result.error){
    			var responseData = result.data.split(",");
  ...
  //filter table data
    $("#filter_table_button").click(function(){
    	var user_id = "1";
    	var filter_code = $("#filter_txt_code").val();
    	var filter_description = $("#filter_txt_desc").val();
    	//alert("filtering: "+filter_code+" :: "+filter_description);
    	
        var request_url = "/site/blocks/shortcodes/ajax/shortcodefunctions.jag";
    	
    	jagg.post(request_url, { action:"FilterCodesTable", 'userid':user_id, 'shortcode':filter_code , 'description':filter_description }, function (result) {
        	//alert(result.data);
...
function loadShortCodesData(){
	var user_id = "1";
	jagg.post("/site/blocks/shortcodes/ajax/shortcodefunctions.jag", { action:"GetTableData", userid:user_id }, function (result) {
    	//alert(result.data);
