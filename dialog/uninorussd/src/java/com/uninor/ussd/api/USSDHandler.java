...
import com.uninor.ussd.api.responsebean.sms.SendSMSRequest;
import com.uninor.ussd.api.responsebean.sms.StartSubscription;
...
import com.uninor.ussd.api.responsebean.ussd.OutboundMessage;
...
import com.uninor.ussd.api.service.RequestManager;
...
public class USSDHandler {
    ...
    private final String SERVICE_TYPE = "WSO2";
    ...
    public Response outboundRequest(@PathParam("senderAddress") String senderAddress, String jsonBody) {
        ...
        try {
            ...
            if (outboundMsg.getOutboundUSSDMessageRequest().getUssdAction() == USSDAction.mtinit) {
                ...
                String UniqueID = application.getShortCode() + System.currentTimeMillis() + "N" + ResourceURLUtil.toHexString(new byte[]{(byte) 0x00, (byte) 0x00});
            ...
            sendMessage(sender, outboundMsg.getOutboundUSSDMessageRequest().getAddress(), outboundMsg.getOutboundUSSDMessageRequest().getOutboundUSSDMessage(), false, outboundMsg.getOutboundUSSDMessageRequest().getSessionID(), id.intValue(), isRegistered, isEndsession, null);
    ...
    public void sendMessage(String port, String phoneNo, String msg, boolean highPriority,
            String msgId, int msgRefNum, boolean isRegistered, boolean isEndSession, byte[] destNodeId) {
        ...
        byte[] dialogId = ResourceURLUtil.toByteArray(msgId.split("N")[1]);
        if (destNodeId == null) {
            destNodeId = "000000".getBytes();
        }
        if (msg.length() > 160) {
            ...
            for (int i = 0; i < msgs.size(); i++) {
                ...
                //sm.setMessageText(ResourceURLUtil.setSpiceMsg(msgs.get(i),Byte.toString(dialogId),(isEndSession)?"0" : "1") );
                sm.setMessageText(msgs.get(i));
                sm.setServiceType(SERVICE_TYPE);
                //Set Message ID
                ...
                sm.setOptionalParameter(Tag.getTag(NodeTag.TAGVALUE_DEST_NODE_ID), destNodeId);

                if (isEndSession) {
                    dialogId[1] = (byte) (dialogId[1] | (byte) 0x01);
                    sm.setOptionalParameter(Tag.ITS_SESSION_INFO, dialogId);
                } else {
                    dialogId[1] = (byte) (dialogId[1] | (byte) 0x00);
                    sm.setOptionalParameter(Tag.ITS_SESSION_INFO, dialogId);
                }
            ...
            //sm.setMessageText(ResourceURLUtil.setSpiceMsg(msg,Byte.toString(dialogId),(isEndSession)? "0" : "1"));
