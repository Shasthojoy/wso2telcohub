 ...
 * @author User
 */
@WebServlet(name = "ACRDeactivateServlet", urlPatterns = {"/ACRDeactivateService"})
public class ACRDeactivateServlet extends ACRServer {

    static Logger logger = Logger.getLogger(ACRDeactivateServlet.class);

    public void init() throws ServletException {
        logger.debug("ACRDeactivateServlet initialised");
    }
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (validateRequest(request, response, requestParts, validationRules)) {
            ...
            try {
                ...
                JSONObject refreshAcrRequest = (JSONObject) refreshJsonObj.get("deactivateAcrRequest");
            ...
            if (checkRequestParameters(response, rule)) {
                try {
                        ...
                        if (Validations.isACRIdExistForAppIdAndACR(applicationId, acr)) {
                            if (Getters.getAppStatus(applicationId) == 1) {
                                if (Getters.getACRStatus(acr) == 1) {
                                    String responseJson = ACRController.deactivateACR(appCode, developerCode, acr);
                    ...
                    logger.debug("ASRDeactivateServlet: " + e);
                }
            }
        }
    }
